...
extension FaceViewController {
    ...
    private func initCameraView() {
        viewCamera = FaceDetectCameraView(bufferQueue: DispatchQueue(label: "Camera"), onResults: { image, checkDetect, maskType, cameraError in
            ...
            if !checkDetect {
                ...
                guard let maskType = maskType else {
                    return
                }
                if maskType == .mask {
                    AlertUtil.showAlert(from: self, with: "Chụp thành công, Ảnh có mặt nạ", message: "") { _ in
                        self.viewCamera.startCapture()
                    }
                } else {
                    AlertUtil.showAlert(from: self, with: "Chụp thành công, Ảnh không có mặt nạ", message: "") { _ in
                        self.viewCamera.startCapture()
                    }
                }
            }
